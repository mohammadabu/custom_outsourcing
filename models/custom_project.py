...
class CustomOutsourcing(models.Model):
    ...
    def calc_new_project_completed(self,id):
        ...
        for task in all_tasks:
            ...
            # work_task = False
            # work_percentage = False
            # assigned_len = len(task.assigned_res)
            # duration = task.duration_1
            # work_task = float(assigned_len * duration)
            # percentage = task.percentage_complete
            # work_percentage = (percentage/ 100) * work_task
            # work += work_task
            # work_complete += work_percentage     
        # res = int((work_complete / work) * 100)
        if len(all_tasks) > 0: 
    ...
    def get_all_employee_position(self,pos_id,all_emails):
        # job_position_test = self.env['hr.job'].sudo().search([])
        # for position_test in job_position_test:
        #     position_test.internal_id = position_test.name
